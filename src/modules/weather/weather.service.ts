...
const normalizeCurrent = (
  ...
  if (!data.current) {
    throw new BadGatewayException('Weather provider returned no current weather data');
  }
...
export class WeatherService {
  ...
  private async getCachedWeather<T>(lat: number, lon: number, type: WeatherCacheType): Promise<T | null> {
    const result = await this.pool.query<WeatherCacheRow>(
  ...
  async getCurrentWeather(lat: number, lon: number): Promise<WeatherCurrentResponse> {
    const cachedWeather = await this.getCachedWeather<WeatherCurrentResponse>(lat, lon, 'current');
    ...
    try {
      ...
      await this.setCachedWeather(lat, lon, 'current', weather, CURRENT_CACHE_TTL_MINUTES);
  ...
  async getForecastWeather(lat: number, lon: number): Promise<WeatherForecastResponse> {
    const cachedWeather = await this.getCachedWeather<WeatherForecastResponse>(lat, lon, 'forecast');
    ...
    try {
      ...
      await this.setCachedWeather(lat, lon, 'forecast', forecast, FORECAST_CACHE_TTL_MINUTES);
  ...
  async getAirQuality(lat: number, lon: number): Promise<AirQualityResponse> {
    const cachedAirQuality = await this.getCachedWeather<AirQualityResponse>(lat, lon, 'air-quality');
    ...
    try {
      ...
      if (aqi === undefined) {
        throw new BadGatewayException('Weather provider returned no air quality data');
      }

      const airQuality = { aqi };
      await this.setCachedWeather(lat, lon, 'air-quality', airQuality, AIR_QUALITY_CACHE_TTL_MINUTES);
